...
namespace QuickBackupX
{
	...
	ZRESULT Backup::Create()
	{
		...
		sendText("all", string("开始压缩 共有 ") + to_string(flsize) + " 个文件待压缩");
		map<string, filesystem::path>::iterator iter = this->flist.begin();
		i = 0;
		for (; iter != this->flist.end(); ++iter)
		{
			i++;
			sendText("all", string("正在压缩第 ") + to_string(i) + " / " + to_string(flsize) + " 个文件");
			Debug{ L_INFO(string("- 添加文件: ") + iter->second.string() + " 至 " + iter->first); }
	...
	bool Backup::CopyLevelToTempDir()
	{
		...
		{
			PRWARN(u8"临时目录(" << TEMPDIR << u8")已经存在!!!");
			L_WARNING("临时目录(" + TEMPDIR + ")已经存在!!!");
			this->DeleteTempDir();
		}
		filesystem::copy(this->lpath, filesystem::path(TEMPDIR));
		Debug{ L_INFO(string("已复制 ").append(this->lpath.string().append(" 至 ").append(TEMPDIR))); }
		L_INFO(string("开始遍历目录:").append(this->lpath.string()));
		Debug{ L_INFO("已构造 std::filesystem::path 对象"); }
		if (!exists(this->lpath))
		{
			PRERR(u8"目录 " << this->lpath.string() << u8" 不存在!!!");
			L_ERROR(string("目录 ") + this->lpath.string() + " 不存在!!!");
			return false;
		}
		if (!exists(filesystem::path(TEMPDIR)))
		{
			PRERR(u8"目录 " << TEMPDIR << u8" 不存在!!!");
			L_ERROR(string("目录 ") + TEMPDIR + u8" 不存在!!!");
			return false;
		}
		filesystem::directory_entry entry(this->lpath);
		Debug{ L_INFO("已构造 std::filesystem::directory_entry 对象"); }
		if (entry.status().type() != filesystem::file_type::directory)
		{
			PRERR(this->lpath.string() << u8" 不是文件夹!!!");
			L_ERROR(this->lpath.string() + " 不是文件夹!!!");
			return false;
		}
		filesystem::directory_iterator list(this->lpath);
		Debug{ L_INFO("已构造 std::filesystem::directory_iterator 对象"); }
		for (auto& it : list)
		{
			if (it.status().type() == filesystem::file_type::directory)
			{
				Debug{ L_INFO(string("- 文件夹: ").append(it.path().string())); }
				filesystem::copy(it.path(), filesystem::path(TEMPDIR + it.path().filename().string()));
				Debug{ L_INFO(string("已复制 ") + it.path().string() + " 至 " + TEMPDIR + it.path().filename().string()); }
			}
		}
		return true;
	}
	...
	{
		Debug{ L_INFO(string("开始删除临时目录: ") + TEMPDIR); }
		if (!exists(filesystem::path(TEMPDIR)))
		{
			PRWARN(u8"临时目录(" << TEMPDIR << u8")不存在!!!");
			L_WARNING("临时目录(" + TEMPDIR + ")不存在!!!");
			return true;
		}
		size_t dfc = filesystem::remove_all(filesystem::path(TEMPDIR));
		Debug{ L_INFO(TEMPDIR + " 已删除,共删除了 " + to_string(dfc) + " 个文件"); }
		return true;
	}
	...
	bool Backup::IterTempDir(string path)
	{
		...
		L_INFO(string("开始遍历目录:").append(tempd));
		Debug{ L_INFO("已构造 std::filesystem::path 对象"); }
		if (!exists(dir))
		{
			PRERR(u8"目录 " << tempd << u8" 不存在!!!");
			L_ERROR(string("目录 ") + tempd + " 不存在!!!");
			return false;
		}
		...
		for (auto& it : list)
		{
			...
			Debug{ L_INFO(string("- 文件: ").append(it.path().string())); }
		}
		return true;
	}
}